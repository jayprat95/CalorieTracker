...
- (void)installTheButton {
    ...
    if(self.tabBar.items.count > self.locationIndexInTabBar) {
        ...
        CGRect myRect = CGRectMake(20, 468, 86, 86);
        self.frame = myRect;
        self.center = buttonLocation;
        self.backgroundColor = [UIColor paperColorRed];
...
- (void)setImage:(UIImage *)image forBROptionsButtonState:(BROptionsButtonState)state {
    if(state == BROptionsButtonStateNormal) {
        [self.closedStateImage setImage:image];
    }
    else {
        [self.openedStateImage setImage:image];
    }
}
...
- (void)showOptions {
    ...
    CGFloat angle = 0.0;
    CGFloat radius = 60 * numberOfItems;
